        ...
        for did in deck_ids:
            try:
                ...
                for sec, raw_name, cnt, url in cards:
                    ...
                    elif "サポート"  in sec: cat = "supporter"
                    elif "スタジアム" in sec: cat = "stadium"
                    elif "どうぐ"    in sec: cat = "tools"
                    elif "グッズ"    in sec: cat = "goods"
                    elif "ポケモン"  in sec: cat = "pokemon"
                    else: continue
                ...
                for _, raw_name, cnt, _ in cards:
                    ...
                    for target in ace_jp_norm_names:
                        if target in nname:
                            ace_occurrences.append((did, nname, ace_targets_raw.get(target, ""), cnt))
                            ace_deck_sets[target].add(did)
                            found_aces.append(raw_name)

                if found_aces:
                    print(f"✓ {did} 完成：{stats} ｜ 找到 ACE SPEC: {found_aces}")
                else:
                    print(f"✓ {did} 完成：{stats}")
